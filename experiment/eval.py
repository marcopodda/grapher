...
def orbit_worker(G):
    try:
        counts = orca(G)
        return np.sum(counts, axis=1).reshape(-1)
...
def normalize(ref_counts, gen_counts, bins=100):
    _min = min(ref_counts.min(), gen_counts.min())
    _max = max(ref_counts.max(), gen_counts.max())
    ref_counts = (ref_counts - _min) / (_max - _min)
    gen_counts = (gen_counts - _min) / (_max - _min)
